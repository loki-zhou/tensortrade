...
from tensortrade.tray_ext.tensortradeExtension.actions.short_hold import SH
...
class SNWC(TensorTradeRewardScheme):
    ...
    def get_reward(self, action_scheme: SH) -> float:
        ...
        """
        asset_balance = action_scheme.agent_asset.balance.convert(action_scheme.exchange_pair)
        cash_balance = action_scheme.cash.balance
        deposit_margin = action_scheme.deposit_margin.balance
        borrowed_cash = action_scheme.short_trade_value.convert(
            action_scheme.exchange_pair)
        net_worth = (
            asset_balance + cash_balance + deposit_margin - borrowed_cash).as_float()
