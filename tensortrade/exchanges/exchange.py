...
class Exchange(Component):
    ...
    def __init__(self, dtype: TypeString = np.float32, feature_pipeline: FeaturePipeline = None, **kwargs):
        ...
        self._min_trade_amount = self.default('min_trade_amount', -np.inf, kwargs)
        self._max_trade_amount = self.default('max_trade_amount', np.inf, kwargs)
        self._min_trade_price = self.default('min_trade_price', -np.inf, kwargs)
        self._max_trade_price = self.default('max_trade_price', np.inf, kwargs)
    ...
    def observation_space(self) -> Box:
        ...
        n_features = len(self.observation_columns)
        return  Box(
            low=self._min_trade_price,
            high=self._max_trade_price,
    ...
    def next_observation(self) -> np.ndarray:
        ...
        observation = observation.astype(self._dtype)
