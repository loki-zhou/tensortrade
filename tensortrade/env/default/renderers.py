...
from tensortrade.env.generic.components.renderer import BaseRenderer
...
class PlotlyTradingChart(BaseRenderer):
    ...
    `height` : int
        Chart height in pixels. Affects both display and saved file charts. Set to `None` for 100% height. Default is `None`.
    `save_format` : str
        A format to save the chart to. Acceptable formats are: `html`, `png`, `jpeg`, `webp`, `svg`, `pdf`, `eps`. All the formats except for `html` require `Orca`. Default is `None` for no saving.
    `path` : str
        The path to save the chart to if save_format is not None. The folder will be created if not found.
    `filename_prefix` : str
        A string that precedes automatically-created file name when charts are saved. Default 'chart_'.
    `timestamp_format` : str
    ...
    def __init__(self,
                ...
                include_plotlyjs: Union[bool, str] = False, #'cdn'
    ...
    def _create_figure(self, performance_keys: dict) -> None:
        if self.render_mode is not None:
            ...
            fig.add_trace(go.Bar(name='Volume', showlegend=False,
                                marker={'color': 'DodgerBlue'}), row=2, col=1)
    ...
    def render_env(
        ...
        if self.render_mode is not None:
            ...
            self._volume_chart.update({'y': price_history['volume']}) # Deactivate this
...
class PositionChangeChart(BaseRenderer):
    """
    Trading visualization for TensorTrade using Plotly.
    ...
    def _create_figure(self, performance_keys: dict) -> None:
        ...
        fig.update_yaxes(linecolor='Grey', gridcolor='Gainsboro')
        fig.update_xaxes(title_text='Price', row=1)
        fig.update_xaxes(title_text='Performance', row=2)
...
class MatplotlibTradingChart(BaseRenderer):
    ...
    def _render_trades(self, step_range, trades) -> None:
        ...
        # The code above is a list comprehension that creates a new list, called "trades", from the values of another list, "trades.values()". The new list contains all the elements from each sublist in the original list.
    ...
    def _render_price(self, step_range, times, current_step) -> None:
        ...
        last_open = self._df['open'].values[current_step]
        last_low = self._df['low'].values[current_step]
    ...
    def render_env(
        ...
        self.fig.suptitle(
            'Net worth: $' + str(current_net_worth) +
...
class PLSHPositionChangeChart(BaseRenderer):
    """ Trading visualization for TensorTrade using Matplotlib
    Parameters
    ---------
    display : bool
    ...
    def _render_trades(self, trades,
        ...
        trades = [i for sublist in actions for i in sublist]
    ...
    def render_env(
        ...
        if len(times) > 0:
            ...
            self._render_price(step_range, times, current_step)
            self._render_volume(step_range, times)
            self._render_trades(step_range, times)
...
class BSH_Long_PositionChangeChart(Renderer):
    """ Trading visualization for TensorTrade using Matplotlib
    ...
    def render(self, env, **kwargs):
        ...
        performance_df = pd.DataFrame().from_dict(env.action_scheme.portfolio.performance, orient='date')
...
_registry = {
    ...
    "plotly_position": PositionChangeChart,
    "matplot": MatplotlibTradingChart,
    "plsh_matplot": PLSHPositionChangeChart,
    "bsh_matplot": BSH_Long_PositionChangeChart,
}
